...
def test_build_mapping_rejects_short_name_collisions():
    with pytest.raises(ValueError, match="short section name collision"):
        sut.build_mapping(["first", "second"], shorten=lambda _name: "zcollision")


def test_write_outputs_contains_compiler_linker_and_alias_metadata(tmp_path):
    names = ["ztest_unit_test", "unused"]
    mapping = sut.build_mapping(names)
    header = tmp_path / "macho_sections.h"
    aliases = tmp_path / "macho_sections.c"
    linker = tmp_path / "macho_sections.ld"

    sut.write_outputs(mapping, header, aliases, linker, alias_names=["ztest_unit_test"])
    ...
    assert short_name in linker.read_text()
