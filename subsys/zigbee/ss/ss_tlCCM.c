...
_CODE_SS_ u8 ss_ccmEncryption(u8 *key, u8 *nonce, u8 nwkHdrLen, u8 *nwkHdr, u8 srcMsgLen, u8 *srcMsg)
{
    ...
    /* Keep the TX path byte-for-byte equivalent to libzigbee. */
    if (aes_ccmAuthTran(AES_CCM_MIC_LEN, key, nonce, srcMsg, srcMsgLen,
                        nwkHdr, nwkHdrLen, mic) != RET_OK) {
        return 0;
    }

    aes_ccmEncTran(AES_CCM_MIC_LEN, key, nonce, srcMsg, srcMsgLen,
                   nwkHdr, nwkHdrLen, mic);
    /* The TC32 optimized memcpy miscompiles some unaligned four-byte tails
     * (the observed symptom is exactly +2 in the final MIC byte).  Keep the
     * libzigbee CCM algorithm and vendor AES sequence, but make this final
     * security boundary explicitly byte-wise. */
    for (u8 i = 0U; i < AES_CCM_MIC_LEN; i++) {
        srcMsg[srcMsgLen + i] = mic[i];
    }
