...
{
	/*
	 * Let normal BDB network-steer learn the PAN/parent from the active-scan
	 * Beacon (the coordinator on this medium), instead of constraining this
	 * router to a fixed target.
	 */
	ARG_UNUSED(target);
	return false;
}
...
int main(void)
{
	LOG_INF("native_sim socket Zigbee router ready");
	return 0;
}
