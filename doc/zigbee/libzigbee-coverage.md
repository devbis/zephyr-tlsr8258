...
| `apsTxDataSendStart` | Copies a payload that lies outside the buffer being duplicated | The vendor radio receives into the same pool, so rebasing the pointer always works there; a router relaying a received frame points into the Zephyr radio's receive ring. |
...
| `zdo_nlme_join_indication` | Registers the child address pair before authentication, leaves `useParent` clear for a coordinator, and enables it only for a router child | The association handoff carries the allocated short address, but the APS Transport-Key path also needs the short↔IEEE map. A coordinator sends the key directly; setting the relay flag on both roles routes it through an uninitialized parent address. |
